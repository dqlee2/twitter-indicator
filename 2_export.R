...
for (iso2 in clist) {
  ...
  tmp_data = read.csv(
    file = paste0("./out/data_iso2_", iso2, ".csv"), 
    header = TRUE, sep = ",", 
...
data_iso2$tm = as.yearmon(
  paste(data_iso2$year, data_iso2$month, sep = "-"), "%Y-%m")
...
data_iso2 = data_iso2[
  ,c("iso2","year","month",var_name)
  ] %>% arrange(iso2,year,month)

write.csv(data_iso2, 
          file = paste0("./out/twitter_country", ".csv"), row.names = FALSE)
...
for (iso2 in clist) {
  ...
  if (iso2=="US") {
    tmp_data = read.csv(
      file = paste0("./out/data_city_", iso2, ".csv"), 
      header = TRUE, sep = ",", 
...
data_city = data_city[
  ...
  c("iso2","metro_area_name","year","month",var_name)
  ] %>% arrange(iso2,metro_area_name,year,month)

write.csv(data_city, 
          file = paste0("./out/twitter_city", ".csv"), row.names = FALSE)
